...
`)
	...
	if cfg.Backends[0].Name != "gpu0" {
		t.Errorf("expected gpu0, got %s", cfg.Backends[0].Name)
	}
	if cfg.Backends[1].Name != "gpu1" {
		t.Errorf("expected gpu1, got %s", cfg.Backends[1].Name)
	}
	if cfg.Backends[1].URL != "http://localhost:8001" {
		t.Errorf("expected port 8001, got %s", cfg.Backends[1].URL)
	}
}
